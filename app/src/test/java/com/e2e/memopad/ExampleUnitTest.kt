...
class ExampleUnitTest {
    ...
    fun add_appendsTrimmedMemo() {
        val result = MemoLogic.add(emptyList(), "  買い物  ", id = 1L)
    ...
    fun add_ignoresBlankInput() {
        val result = MemoLogic.add(emptyList(), "   ", id = 1L)
        assertTrue(result.isEmpty())
    }

    @Test
    fun add_isImmutable() {
        val original = listOf(Memo(1L, "a"))
        val result = MemoLogic.add(original, "b", id = 2L)
    ...
    fun remove_dropsMatchingId() {
        val memos = listOf(Memo(1L, "a"), Memo(2L, "b"))
